...
public class Controller extends UnicastRemoteObject implements CatalogServer {
	...
	@Override
	public synchronized boolean downloadFile(CatalogServer catalogServer, String name) throws RemoteException {
		// Check if file exists for security
		this.clientHandlers.get(catalogServer).sendFile(name);
		return true;
	}

	@Override
	public synchronized boolean removeFile(CatalogServer catalogServer, String name) throws RemoteException {
		boolean dataDeleted = this.catalog.deleteFile(name, this.loggedUsers.get(catalogServer));
	...
	public synchronized boolean updateFile(CatalogServer catalogServer, String fileName, long length, String access,
			String action) throws RemoteException {
		return this.catalog.updateFile(fileName, length, this.loggedUsers.get(catalogServer), access, action);
	}
